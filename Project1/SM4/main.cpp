...
int main() {
    ...
    for (int i = 0; i < 16; ++i) {
        std::cout << std::hex << std::setw(2) << std::setfill('0')
            << static_cast<int>(key_bytes[i]);
    }
    std::cout << std::endl;

    uint8_t ciphertext_bytes[16];
    for (int i = 0; i < 4; ++i) {
        word_to_bytes(ciphertext[i], ciphertext_bytes + 4 * i);
    }
    std::cout << "密文: ";
    for (int i = 0; i < 16; ++i) {
        std::cout << std::hex << std::setw(2) << std::setfill('0')
            << static_cast<int>(ciphertext_bytes[i]);
    }
    std::cout << std::endl;

    uint8_t decrypted_bytes[16];
    for (int i = 0; i < 4; ++i) {
        word_to_bytes(decrypted[i], decrypted_bytes + 4 * i);
    }
    std::cout << "解密结果: ";
    for (int i = 0; i < 16; ++i) {
        std::cout << std::hex << std::setw(2) << std::setfill('0')
